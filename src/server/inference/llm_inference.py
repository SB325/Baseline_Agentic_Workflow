...
class VLLMSingleton:
    '''
    This class runs the vllm inference engine. It is configured to only run a single 
    instance that all clients share. By only running a single llm inference engine, 
    more GPU resources are reserved for a larger K-V cache or other
    '''
    _instance = None
    @classmethod
    def get_engine(cls):
